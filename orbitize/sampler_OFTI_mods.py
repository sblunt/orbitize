...
class OFTI(Sampler,):
    ...
    def __init__(self, system, like='chi2_lnlike', custom_lnlike=None):
        ...
        if len(self.system.rv[0]) > 0 and self.system.fit_secondary_mass:
            self.rv_observed = self.system.data_table[:]['quant1'].copy()
            self.rv_err = self.system.data_table[:]['quant1_err'].copy()
    ...
    def prepare_samples(self, num_samples):
        ...
        if len(self.system.rv[0]) > 0 and self.system.fit_secondary_mass:
            ...
            v_star = vc*-(m1/m0)
            v_diff = abs(v_star[0] - v_star[1])  # proxy Kamp for model
            ...
            gamma_obs = self.rv_observed[self.epoch_rv_idx[1]] - \
                vstar[1]  # difference between observed and model to shift
