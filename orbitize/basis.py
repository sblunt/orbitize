...
class Basis(abc.ABC):
    ...
    def __init__(
        ...
        self.default_priors = {
            ...
            "ecc": priors.UniformPrior(0.0, 1.0),
            "inc": priors.SinPrior(),
            "aop": priors.UniformPrior(0.0, 2.0 * np.pi),
    ...
    def set_hip_iad_priors(self, priors_arr, labels_arr):
        ...
        priors_arr.append(
            priors.UniformPrior(
                -10 * self.hipparcos_IAD.alpha0_err, 10 * self.hipparcos_IAD.alpha0_err
            )
        )
        ...
        priors_arr.append(
            priors.UniformPrior(
                -10 * self.hipparcos_IAD.delta0_err, 10 * self.hipparcos_IAD.delta0_err
            )
        )
    ...
    def set_rv_priors(self, priors_arr, labels_arr):
        ...
        for instrument in self.rv_instruments:
            ...
            priors_arr.append(
                priors.LogUniformPrior(1e-4, 0.05)
            )  # jitter prior in km/s
    ...
    def set_default_mass_priors(self, priors_arr, labels_arr):
        ...
        if self.mass_err > 0:
            priors_arr.append(
                priors.GaussianPrior(self.stellar_or_system_mass, self.mass_err)
            )
        else:
...
class SemiAmp(Basis):
    ...
    def construct_priors(self):
        ...
        for body_num in np.arange(self.num_secondary_bodies) + 1:
            ...
            self.standard_basis_idx["m{}".format(body_num)] = self.param_idx[
                "K{}".format(body_num)
            ]
    ...
    def verify_params(self):
        ...
        if not self.fit_secondary_mass:
            warnings.warn(
                "This basis will not sample total mass. It will sample stellar mass."
            )
...
class XYZ(Basis):
    ...
    def construct_priors(self):
        ...
        for body in np.arange(self.num_secondary_bodies):
            ...
            best_epochs = self.epochs[
                best_idx : (best_idx + datapoints_to_take)
            ]  # 0 is best, the others are for fitting velocity
            ...
            best_ras = self.data_table["quant1"][
                best_idx : (best_idx + datapoints_to_take)
            ].copy()
            best_ras_err = self.data_table["quant1_err"][
                best_idx : (best_idx + datapoints_to_take)
            ].copy()
            best_decs = self.data_table["quant2"][
                best_idx : (best_idx + datapoints_to_take)
            ].copy()
            best_decs_err = self.data_table["quant2_err"][
                best_idx : (best_idx + datapoints_to_take)
            ].copy()
            ...
            best_xs_err = np.sqrt(
                (best_ras_err / best_ras) ** 2 + (self.plx_err / self.plx) ** 2
            ) * np.absolute(best_xs)
            best_ys_err = np.sqrt(
                (best_decs_err / best_decs) ** 2 + (self.plx_err / self.plx) ** 2
            ) * np.absolute(best_ys)
            ...
            z_bound = (
                np.sqrt(mu_vel**2 - (best_xs[0] ** 2 + best_ys[0] ** 2) * u.AU * u.AU)
            ).to(u.AU)
            z_bound = z_bound.value

            mu_pos = (
                2 * mu / np.sqrt((best_xs[0] ** 2 + best_ys[0] ** 2) * (u.AU * u.AU))
            )
            z_vel_bound = (
                np.sqrt(mu_pos - (x_vel**2 + y_vel**2) * (u.km / u.s * u.km / u.s))
            ).to(u.km / u.s)
            ...
            basis_priors.append(
                priors.UniformPrior(
                    best_xs[0] - num_uncerts_x * best_xs_err[0],
                    best_xs[0] + num_uncerts_x * best_xs_err[0],
                )
            )
            ...
            basis_priors.append(
                priors.UniformPrior(
                    best_ys[0] - num_uncerts_y * best_ys_err[0],
                    best_ys[0] + num_uncerts_y * best_ys_err[0],
                )
            )
            ...
            basis_priors.append(
                priors.GaussianPrior(0.0, z_bound / 4, no_negatives=False)
            )
            ...
            basis_priors.append(
                priors.UniformPrior(
                    x_vel - num_uncerts_xvel * x_vel_err,
                    x_vel + num_uncerts_xvel * x_vel_err,
                )
            )
            ...
            basis_priors.append(
                priors.UniformPrior(
                    y_vel - num_uncerts_yvel * y_vel_err,
                    y_vel + num_uncerts_yvel * y_vel_err,
                )
            )
            ...
            basis_priors.append(
                priors.GaussianPrior(0.0, z_vel_bound / 4, no_negatives=False)
            )
        ...
        for body_num in np.arange(self.num_secondary_bodies) + 1:
            ...
            self.standard_basis_idx["ecc{}".format(body_num)] = self.param_idx[
                "y{}".format(body_num)
            ]
            self.standard_basis_idx["inc{}".format(body_num)] = self.param_idx[
                "z{}".format(body_num)
            ]
            self.standard_basis_idx["aop{}".format(body_num)] = self.param_idx[
                "xdot{}".format(body_num)
            ]
            self.standard_basis_idx["pan{}".format(body_num)] = self.param_idx[
                "ydot{}".format(body_num)
            ]
            self.standard_basis_idx["tau{}".format(body_num)] = self.param_idx[
                "zdot{}".format(body_num)
            ]
    ...
    def xyz_to_standard(self, epoch, elems, tau_ref_epoch=58849):
        ...
        cos_pan = (
            np.sum(n * unit_j, axis=0) / n_magnitude
        ).value  # take dot product with i?
        ...
        cos_tanom = (
            np.sum(pos * e_vector, axis=0) / (pos_magnitude * e_vec_magnitude)
        ).value
        ...
        time_tau = epoch - ((np.sqrt(sma**3 / mu)).to(u.day)).value * (
            eanom - ecc * np.sin(eanom)
        )
    ...
    def standard_to_xyz(
        ...
        # Mean anomaly
        manom = (mean_motion * (epoch - tau_ref_epoch) - 2 * np.pi * tau) % (
            2.0 * np.pi
        )
        # Eccentric anomaly
        ...
        vel_peri_x = -(
            (
                np.sqrt(consts.G * (mtot * u.Msun) * (sma * u.AU))
                * np.sin(eanom)
                / (pos_magnitude * u.AU)
            ).to(u.km / u.s)
        ).value
        ...
        T = np.array([[T_11, T_12, T_13], [T_21, T_22, T_23], [T_31, T_32, T_33]])
    ...
    def verify_params(self):
        ...
        for i in range(self.num_secondary_bodies):
            ...
            if max_index - best_epoch_loc < 2:
                raise Exception(
                    "For now, the epoch with the lowest sepparation error should not be one of the last two entries for body{}".format(
                        body_num
                    )
                )
