...
def test_compute_model():
    """
    Tests that the perturbation of a second planet using compute model gives
    ...
    # create the orbitize system and generate model predictions using the standard 1
    # body model for b, and the 2 body model for b and c
    astrom_dat = read_input.read_file(filename)

    sys_1body = system.System(
        1, astrom_dat, m0, plx, tau_ref_epoch=tau_ref_epoch, fit_secondary_mass=True
    )
    sys_2body = system.System(
        2, astrom_dat, m0, plx, tau_ref_epoch=tau_ref_epoch, fit_secondary_mass=True
    )

    # model predictions for the 1 body case
    # we had put one measurements of planet b in the data table, so compute_model
    ...
    # just test the first value
    # because of the face on orbit, we should see it immediately.
    assert total_diff[0] == pytest.approx(
...
def test_fit_selfconsist():
    ...
    _ = np.sqrt(c_params[0] ** 3 / mtot_c)

    epochs = (
        np.linspace(0, period_b * 365.25, 20) + tau_ref_epoch
    ...
    samp.run_sampler(n_walkers * 150, burn_steps=125)

    res = samp.results

    print(
        np.median(res.post[:, sys.param_idx["m1"]]),
