...
def test_period_basis():
    ...
    my_system = system.System(
        1, data_table, 1.75, 51.44, mass_err=0.05, plx_err=0.12, fitting_basis="Period"
    )
...
def test_xyz_basis():
    ...
    locs = np.where((conversion[1, :] >= 1.0) | (conversion[1, :] < 0.0))[0]
    ...
    sample_copy = samples.copy()
    conversion = my_system.basis.to_standard_basis(samples)
    locs = np.where((conversion[[1, 7], :] >= 1.0) | (conversion[[1, 7], :] < 0.0))[1]
    locs = np.unique(locs)
    sample_copy = np.delete(sample_copy, locs, axis=1)
