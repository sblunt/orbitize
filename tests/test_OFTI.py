...
def test_scale_and_rotate():
    ...
    ra, dec, vc, _ = orbitize.kepler.calc_orbit(
...
def test_run_sampler():
    ...
    seppa_lnprob_compare = (
        orbits[0],
        computed_lnlike_test,
    )  # one set of params and associated lnlike saved.
...
def test_not_implemented():
    ...
    try:
        ...
        mySystem = system.System(
            1, data_table, 1.22, 56.95, mass_err=0.08, plx_err=0.26, hipparcos_IAD=myHip
        )
...
def test_fixed_sys_params_sampling():
    ...
    myDriver = orbitize.driver.Driver(input_file, "OFTI", 1, 1.22, 56.95)
...
def test_OFTI_multiplanet():
    ...
    myDriver = orbitize.driver.Driver(
        input_file, "OFTI", 2, 1.52, 24.76, mass_err=0.15, plx_err=0.64
    )
...
def test_OFTI_covariances():
    """
    Test OFTI fits by turning sep/pa measurements to RA/Dec measurements with
    covariances
    ...
    for row in data_table:
        raerr, decerr, radec_corr = system.transform_errors(
            row["quant1"],
            row["quant2"],
            row["quant1_err"],
            row["quant2_err"],
    ...
    assert ref_lnlike == pytest.approx(
        computed_lnlike_ref, abs=0.05
    )  # 5% differencesin lnprob is allowable.
