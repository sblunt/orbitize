...
def test_hipparcos_api():
    ...
    rejected_scansHip = HipparcosLogProb(
...
def test_dvd_vs_2021catalog():
    ...
    assert (
        np.abs(new_iadHipLogProb.plx0 - old_iadHipLogProb.plx0) < 1e-3
    )  # (plx precise to 0.01)
    ...
    assert np.all(
        np.isclose(new_iadHipLogProb.cos_phi, old_iadHipLogProb.cos_phi, atol=1e-2)
    )
    assert np.all(
...
def test_save_load_dvd():
    """
    Set up a Hip IAD + Gaia fit using a DVD file, save the results, and load them.
    """
...
def test_save_load_2021():
    """
    Set up a Hip IAD + Gaia fit using a 2021 file, save the results, and load them.
    """
